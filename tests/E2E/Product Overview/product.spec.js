...
test.describe('Product Overview Tests', () => {
    ...
    test('select random category and assert category title', 
        async ({ page, homePage }) => {
    ...
    test('viewing of product name, price and availability', 
        async ({ page, homePage, productOverviewPage }) => {
    ...
    test('able to add to cart in stock product', 
        async ({ page, productOverviewPage }) => {
            ...
            if (availabilityText.toLowerCase().includes('in stock')) {
                ...
                const cartItemCount = page.locator('a[href="/cart"] > span');
